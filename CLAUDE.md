...
│   ├── pipeline/           # Fetch → normalize → diff → store; parsers/ (one per source),
│   │                       # contracts/ (the zod rows schema.ts mirrors), judge/ (the one writer)
...
│   └── utils/              # site.ts (URL identity), schema.ts (JSON-LD builders), fleet-manifest.ts
│                           # (fleet.json shape), terminal-types.ts (the API payload types)
├── sources.yaml            # Source registry — axis, diff mechanism, include/exclude verdict per source
├── fixtures/               # Captured source bytes + manifest.json (fetch times) — parsers test offline
├── test/                   # vitest units; test/a11y + test/csp are the Playwright browser suites
├── docs/                   # source-audit.md — every candidate source, fetched and judged
