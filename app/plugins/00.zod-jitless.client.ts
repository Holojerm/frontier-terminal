...
//
// That noise is not harmless. A benign refusal on every page load is
// indistinguishable from a real CSP failure, so the first genuine violation
// this app ever has will be the one nobody looks at.
//
...
// ALPHABETICAL order and imports them in that order, so any plugin that builds
// a `z.object(...)` at ITS module scope and sorts before this file constructs
// the schema first — the probe runs and is memoised before `z.config()`
// executes, and this plugin runs, logs, and does nothing. (That is exactly what
// happened in the template this came from, when this file was named
// `zod-jitless.client.ts` and sorted last.) The numeric prefix is Nuxt's
// documented ordering mechanism and is what actually makes this work. Do not
// rename it without checking `bunx playwright test --project=csp`, which is the
...
//
// The cost here is a slower validation path for large schemas. Ours are query
// strings; the interpreter is not the bottleneck.
